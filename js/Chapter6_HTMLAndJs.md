...
话说这么多，怎么在HTML里面插入JavaScript代码？很简单，只需要使用```<script></script>```标签即可。在这个标签内书写你的代码，默认就会被浏览器当做JavaScript代码运行。当然，就标准而言，你还需要在标签内声明对应的类型为“javascript”。

另外，JavaScript标签可以有多个，而且会顺序执行对应的代码。比如在这里我们会演示两处JavaScript代码：
...
保存后用浏览器打开这个网页的话，如果先后得到两次弹窗，就说明你成功运行了JavaScript~

那么，如何操作网页内的元素？如何实现交互？

###4.2 按钮被点击后的事情：点击就送，屠勇者宝刀

按钮按下了，应该发生什么事情？这里可以提及两种思路实现：

1. 先声明一个JavaScript的方法，然后给按钮写一个属性：当按钮被按下的时候，让这个按钮去执行这个方法；
2. 在网页加载完成后，给按钮添加一个属性：被按下时，执行某些方法。

当然了，现在这两种思路看起来大同小异，甚至你会觉得……这不是同一种方法的不同说法吗？其实不然。等你以后了解更多的设计模式有关的知识之后，你就会发现第二种方法可以让网页跟JavaScript代码隔离开来，从而保证了代码的可读性和页面的简洁性。
...
```html
<input type="button" value="领取屠勇者宝刀" onclick="alert('你获得了屠勇者宝刀x1');" />
```

*ps:如果你想试试这条代码的效果，可以只写这一句，直接保存到一个新的HTML文件中后运行该文件。*

当你点击了这个按钮之后，立马就会得到一个……

> ![alert in html](srcImg/chapter6_alertInHTML.png)
> 哈哈，是不是扑面而来一股网页游戏广告的感觉？

不过这就意味着，onclick属性的内容其实就是一段可执行的js代码（默认如此）。当你需要响应的代码是很长一段的js代码时，方法的作用性就体现出来了……

比如史莱姆想要获得10个赞，每次点击时，增加1个赞，满10个之后，即可获得真正的“宝刀”。我们会这么写：先声明方法，再让按钮被点击时执行该方法。
...
<head>
	...
	<script type="text/javascript">
		...
		function AddLike( ) 
		{
			...
			if( likeCount >= 10 )
			{
				alert("众志成城，史莱姆获得了一把屠勇者宝刀（攻击力+9）！");
			}

			console.log("当前点赞数为："+ likeCount );
		}

		//声明方法：显示当前的点赞数
		function ShowLikes ( )
		{
			alert("当前点赞："+likeCount);
		}
	</script>
</head>
<body>
	<p>
		史莱姆：可恶的勇者席卷了我们的居住地，把我们的弟兄们都干掉了。各位父老乡亲，我想复仇！只要给我点满十个赞，我就能获得由XX沐浴露提供的屠勇者宝刀（攻击力+9）。
	</p>
	<p>
		<b onclick="AddLike( )">点赞</b>
		<input type="button" value="统计" onclick="ShowLikes( )" />
	</p>
</body>
</html>
```

上面的代码有什么奇怪的地方么？我们先声明了两个方法，然后，在button中尝试去调用他们。

——但是，慢着，“点赞”似乎只是两个加粗的文字啊？是不是写错了？难道一段普通的文字也可以被点击？

你的回答是正确的。尽管你把鼠标挪上去，在“点赞”上面点击几次，似乎不会发生什么事情，但是，如果你调出我们熟悉的“控制台”窗口，你就会看到你每点击一次，控制台就会输出一次当前的点赞数目：

> ![Like](srcImg/chapter6_AmazingTextCanClick.png)
> 最后，当你点满十次及以上，每点击一次还会弹出一个窗口。

事实上，在HTML中，任何看得到的东西的标签，包括图片、文字、链接都可以拥有“onclick”属性，这样一来，在该元素被点击的时候，就会触发对应的代码。

那么，能否在代码里更改不同元素的属性呢？答案是肯定的，这也是第二种实现的思路：在代码里设置不同元素触发的事件。

然而在此之前，必须要先了解JavaScript的一个重要方法：
...
###4.3 使用JavaScript更改HTML：你以为你以为的就是你以为的吗？
