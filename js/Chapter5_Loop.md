...
> ![Let us dance](srcImg/danceDog.gif) "耶，Let's dance~这就是循环"
...
谈及之前的递归，总是不得不让人提及一个问题：边界。所谓边界，也就是递归该知道自己终止的地方在哪里。——而很多人在初次使用时，总会忘记在适当的时机返回，或者在错误的地方返回。比如：
...
认真仔细地看，你会发现这是一个无穷递归函数，约等于“死循环”。

无穷递归在一般情况下是个危害系数小于死循环的事件，因为一般而言，在调用方法时，计算机会为方法在内存中开辟一段空间，用于保存对应的临时变量和一些参数（这也就是为什么方法里可以声明同名变量而且不会相互影响的原因）。一旦保存方法状态的空间超出限度，程序就会崩溃退出。

然而死循环则不：一般而言，循环很少会有大量申请内存空间的行为，就算有，也要归功于程序有足够智能的“垃圾回收”，也就是上一轮的无关内容有可能会在适当的时机被清理出内存。这也就导致了，一旦程序进入死循环，就将处于“无响应”状态，也就是前面所说的——“卡死”。一旦出现这种状况，你也就只有强行中断整个程序了。**当然如果是在二十年前，你就只能选择断电。**

## Q&A: 提问时间
###1. 方法是不是也可以用中文命名？我看到上一章的方法里似乎可以用中文声明“方法”？方法名的规则是不是也跟变量差不多？方法是不是也可以覆盖原有的方法？

嗯，方法可以用中文名。——但是这是不怎么规范的行为，所以更推荐使用英文命名。

方法与变量的命名规则是：只能以下划线“_”、美元符号“$”、或者字母开头，不允许空格掺杂其中，不过在这之后则可以跟上数字。比如以下的命名就是很不错而且易懂的方法名：
...
这里的建议是，有意义的单词最好是以大写开头，遇到新单词也是如此，这叫啥来着？驼峰命名法则，毕竟这样比：killenemy易识别得多。

另外，方法名是对大小写敏感的，也就是说以下是两种不同的方法：
...
仅仅是开头的字母不同，也将视为两个不同的方法。

另外，需要划重点的是：**相同作用域下的方法可以被覆盖！**

也就是说，如果你不慎声明了两个方法，而且这两个方法恰好名字相同（但是你预期他们的行为却不尽然相同），那么悲剧就出现了。

或者，可能不小心重写了一些现有的方法，那么，恭喜你，你创造了一个无声的bug：

```
alert("这个是现有的alert函数，但是不可以正常弹出窗口。");

//...

//你突然想编写一个向总部发出警告的函数
function alert( msgToSend )
{
	//向总部发起警告：
	var msg = "警告！有状况！";
	//某些函数：
...
你会发现，你无法再使用之前的能弹出窗口的alert函数了。为什么？因为在这个窗口下，你已经覆盖了原来的方法了，很抱歉，你只有重开窗口了。
关于作用域，也许我们在后面会做出说明，但是这里，你只需要知道这个问题的存在就行了。

——**但是，事实上，这不是个问题，而是个很有意义的功能。**

###2. 为什么要有死循环？如果死循环会卡机，那么操作系统有没有检测死循环的功能？

死循环有其存在的必要性。比如，你并不知道一个发往互联网上某台电脑的消息是否被对方收到了，那么，你就只有不断地循环等待，直到某个时间点收到回信你才尝试break掉当前循环。当然，也有可能你永远等不到回应了。

那么，对于另外一些程序，如果不小心出现了永远也不会满足退出条件的死循环，怎么办？操作系统有没有检测死循环的功能呢？很抱歉，没有。所以只有靠你认真仔细地中断程序，然后查错、纠错了。